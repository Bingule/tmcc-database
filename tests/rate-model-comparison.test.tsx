...
describe("compareRateModels", () => {
  ...
  it("serializes comparison statistics and observation-aligned residuals without inventing failed values", async () => {
    ...
    const comparison = serializeModelComparisonCsv(result);
    const residuals = serializeModelComparisonResidualsCsv(data, result);

    expect(comparison.split("\r\n")[0]).toContain("model_id,equation_type,parameters,parameter_count,r_squared,adjusted_r_squared,rmse,aic,aicc,bic,criterion,delta_criterion,convergence,rank,failure_code");
    ...
    expect(residuals).toContain("tian-characteristic-time,0.1,300,298,2");
...
describe("EmpiricalModelsPage", () => {
  it("renders registry-driven validated and pending cards without executable pending claims", async () => {
    ...
    expect(view.querySelectorAll(".rate-model-card-validated a")).toHaveLength(4);
    ...
    for (const label of ["Equation", "Parameters", "Required input", "Useful regime", "Limitations", "Primary reference"]) {
  ...
  it("localizes the complete library in Chinese without exposing raw keys", async () => {
    ...
    for (const text of ["经验模型", "科学验证待定", "适用区间", "主引用", "使用此模型", "比较模型"]) {
