...
export function CvKineticsPage() {
  ...
  const [errorCode, setErrorCode] = useState<CvParseErrorCode | "noOverlap" | "noBFit" | "analysis" | null>(null);
  ...
  const selectedB = nearestBValue(analysis?.bValues ?? [], selectedPotential);
  ...
  const fitChart = makeFitChart(selectedB, t("cv.dunn.measured"));
  ...
  return (
    <section className="tools-page cv-page">
      ...
      <section className="tool-section cv-dunn-analysis">
        ...
        <label>{t("cv.results.rate")} (mV/s)<select value={selectedRate ?? ""} onChange={(event) => setSelectedRate(Number(event.target.value))}>
        ...
        <ScientificLineChart title={t("cv.dunn.chart")} xLabel={`${t("cv.table.potential")} (V)`} yLabel={t("cv.table.current")}
          emptyLabel={t("cv.chart.empty")} legendLabel={t("cv.chart.legend")} series={dunnChart} selectedX={selectedPotential} onSelectX={setSelectedPotential} exportId="cv-dunn-chart" />
        <DataTable headers={[`${t("cv.table.potential")} (V)`, t("cv.dunn.measured"), t("cv.dunn.capacitive"), t("cv.dunn.diffusion")]}
      ...
      <section className="tool-section cv-results">
        ...
        <DataTable headers={[`${t("cv.table.scanRate")} (mV/s)`, t("cv.dunn.capacitive") + " (%)", t("cv.dunn.diffusion") + " (%)"]}
      ...
      <section className="tool-section cv-export">
        ...
        {csvFiles.map((filename) => <button key={filename} type="button" disabled={!valid} onClick={() => analysis && exportCsv(filename, analysis)}>{filename}</button>)}
        <h3>{t("cv.export.figures")}</h3>
        {(["cv-b-chart", "cv-fit-chart", "cv-dunn-chart"] as const).flatMap((id) => [
          <button key={`${id}-svg`} type="button" disabled={!valid} onClick={() => exportFigure(id, "svg")}>{t("cv.export.svg")} — {id}.svg</button>,
          <button key={`${id}-png`} type="button" disabled={!valid} onClick={() => void exportFigure(id, "png")}>{t("cv.export.png")} — {id}.png</button>
        ])}
      </section>
    </section>
  );
...
function errorMessage(code: CvParseErrorCode | "noOverlap" | "noBFit" | "analysis", t: ReturnType<typeof useI18n>["t"]) {
  const keys = {
    ...
    noBFit: "cv.error.noBFit", analysis: "cv.error.analysis"
  } as const;
  return t(keys[code]);
}
...
function DataTable({ headers, rows }: { headers: string[]; rows: Array<Array<string | number | null>> }) {
  ...
  return <div className="tool-table-wrap"><table><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr></thead>
...
function nearestBValue(points: BValuePoint[], selected: number | undefined) {
  if (points.length === 0) return undefined;
  if (selected === undefined) return points[0];
  return points.reduce((nearest, point) => Math.abs(point.potential - selected) < Math.abs(nearest.potential - selected) ? point : nearest);
}
...
function exportCsv(filename: typeof csvFiles[number], analysis: AnalysisState) {
  const { grid, bValues, dunn } = analysis;
  let csv: string;
  if (filename === csvFiles[0]) csv = rowsToCsv(["Potential (V)", ...grid.scanRates.map((rate) => `Current ${rate} mV/s`)], grid.potentials.map((potential, index) => [potential, ...grid.currents.map((row) => row[index])]));
  else if (filename === csvFiles[1]) csv = rowsToCsv(["Potential (V)", "b", "Intercept", "R2", "Point count"], bValues.map((point) => [point.potential, point.b, point.intercept, point.rSquared, point.pointCount]));
  else if (filename === csvFiles[2]) csv = rowsToCsv(["Potential (V)", "k1", "k2", "R2", "Point count"], dunn.points.map((point) => [point.potential, point.k1, point.k2, point.rSquared, point.pointCount]));
  else if (filename === csvFiles[5]) csv = rowsToCsv(["Scan rate (mV/s)", "Capacitive (%)", "Diffusion-controlled (%)"], dunn.contributions.map((item) => [item.scanRate, item.capacitivePercent, item.diffusionPercent]));
  else {
    const capacitive = filename === csvFiles[3];
    csv = rowsToCsv(["Potential (V)", ...dunn.contributions.map((item) => `${item.scanRate} mV/s`)], grid.potentials.map((potential, index) => [potential, ...dunn.contributions.map((item) => (capacitive ? item.capacitiveCurrent : item.diffusionCurrent)[index])]));
  }
