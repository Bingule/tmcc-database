...
export function CaAnalysisResults({ input, options, reconstruction, fit, metadata, onExportError }: {
  ...
  const sampledRate = sampleRateChartPoints(reconstruction.reconstructedRatePoints.map((point) => ({ id: point.id, x: point.rate as number, y: point.capacity })), MAX_DISPLAY_POINTS);
  const sampledFit = sampleRateChartPoints(reconstruction.ratePoints.map((point) => ({ id: point.id, x: point.rate as number, y: point.capacity })), MAX_DISPLAY_POINTS);
  ...
  return <section className="ca-results-workspace">
    ...
    <div className="ca-chart-grid">
      <RateChartPanel title={t("rate.ca.chart.current")} xLabel={t("rate.ca.axis.time", { unit: options.timeUnit })} yLabel={t("rate.ca.axis.current", { unit: options.currentUnit })} rawPointCount={rawCount} displayedPointCount={displayCount} series={[{ id: "ca-current-raw", label: t("rate.ca.chart.rawCurrent"), color: "#607d8b", points: sampled.map(({ point }) => ({ x: point.originalTime, y: point.originalCurrent })) }, { id: "ca-current-adjusted", label: t("rate.ca.chart.adjustedCurrent"), color: "#1f6f78", points: sampled.map(({ point }) => ({ x: point.originalTime, y: point.adjustedCurrentMa / (options.currentUnit === "A" ? 1000 : 1) })) }]} />
      ...
      <RateChartPanel title={t("rate.ca.chart.capacityRate")} xLabel={t("rate.ca.axis.rate")} yLabel={t("rate.ca.axis.capacity")} xScale="log10" exportId="ca-rate-reconstruction-chart" rawPointCount={reconstruction.reconstructedRatePoints.length} displayedPointCount={sampledRate.length} series={[{ id: "ca-rate-capacity", label: "Q(R)", color: "#7b4f9d", mode: "points", points: sampledRate }]} />
      {fit ? <RateChartPanel title={t("rate.ca.chart.fit")} xLabel={t("rate.ca.axis.rate")} yLabel={t("rate.ca.axis.capacity")} xScale="log10" exportId="ca-rate-fit-chart" rawPointCount={reconstruction.ratePoints.length} displayedPointCount={sampledFit.length} metadata={t("rate.ca.chart.metadata")} series={[{ id: "ca-observed", label: t("rate.ca.chart.reconstructed"), color: "#1f6f78", mode: "points", points: sampledFit }, { id: "ca-fit", label: t("rate.ca.chart.fitted"), color: "#c65d26", points: curve }]} /> : null}
    </div>
