  ...
  return <section className="tool-section energy-summary-input">
    ...
    <div className="energy-summary-list">{samples.map((sample, index) => <article className="energy-summary-sample" key={sample.id} aria-labelledby={`${sample.id}-heading`}>
      ...
      <div className="energy-input-grid">
        ...
        <label>{t("rate.energy.input.capacityUnit")}<select value={sample.capacityUnit} onChange={(event) => replace(sample.id, "capacityUnit", event.target.value as EnergyCapacityUnit)}><option value="mAh-g-1">mAh g^-1</option><option value="Ah-kg-1">Ah kg^-1</option><option value="mAh">mAh</option></select></label>
