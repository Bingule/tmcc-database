...
function makeSource(fileName: string, sheetName: string, sheet: Readonly<TabularSheet>, structure: ReturnType<typeof inspect>, headerMode: HeaderMode, mapping: Mapping, mode: EnergyCurveMode): EnergyCurveUploadSource { const offset = structure.hasHeader ? 1 : 0; const column = (index: number) => index >= 0 ? { index, name: structure.headers[index] ?? "" } : null; return { kind: "upload", fileName, sheetName, headerMode, hasHeader: structure.hasHeader, mapping: { x: column(mapping.x), voltage: column(mapping.voltage), current: mode === "time" ? column(mapping.current) : null }, rawRows: structure.dataRows.map((cells, index) => ({ rowNumber: index + offset + 1, cells: [...cells] })) }; }
function failedSource(fileName: string, headerMode: HeaderMode): EnergyCurveUploadSource { return { kind: "upload", fileName, sheetName: "", headerMode, hasHeader: false, mapping: { x: null, voltage: null, current: null }, rawRows: [] }; }
