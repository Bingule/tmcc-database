...
export function ModelComparisonResults({ input, normalized, result, chart, onChartChange, visibleModels, onToggleModel, onExportError }: {
  ...
  const metadata = { modelId: "model-comparison", rateDefinition: "Measured rate R in h^-1", normalizationBasis: "R in h^-1; Q in mAh g^-1", settings: { criterion } } as const;
  const csvItems: RateCsvExportItem[] = [
    ...
    { id: "comparison", label: t("rate.modelComparison.exportComparison"), filename: "rate-model-comparison.csv", csv: serializeModelComparisonCsv(result) },
    { id: "residuals", label: t("rate.analysis.exportResiduals"), filename: "rate-model-residuals.csv", csv: serializeModelComparisonResidualsCsv(fitData, result) },
  ];
  ...
  return <div className="rate-comparison-results">
    ...
    <section className="tool-section rate-comparison-charts">
      ...
      <div className="rate-input-actions" role="tablist" aria-label={t("rate.modelComparison.chartTabs")}>
        <button type="button" role="tab" aria-selected={chart === "capacity"} onClick={() => onChartChange("capacity")}>{t("rate.modelComparison.chart.capacity")}</button>
        <button type="button" role="tab" aria-selected={chart === "residuals"} onClick={() => onChartChange("residuals")}>{t("rate.modelComparison.chart.residuals")}</button>
      </div>
...
function ComparisonChartPanel({ chart, rows, normalized, visibleModels }: {
  ...
  const observed: ChartSeries = { id: "comparison-observed", label: t("rate.modelComparison.chart.observed"), points: normalized.map(({ id, analysisRate: x, analysisCapacity: y }) => ({ id, x, y })), color: "#17242a", mode: "points" };
...
function residualSeries(rows: ReadonlyArray<Readonly<ModelComparisonRow>>, normalized: ReadonlyArray<Readonly<NormalizedRatePoint>>, t: ReturnType<typeof useI18n>["t"]): ChartSeries[] {
  ...
  return [
    ...rows.flatMap((row, index) => row.residuals ? [{ id: `comparison-residual-${row.modelId}`, label: translatedRateModelName(row.modelId, t), points: normalized.map((point, pointIndex) => ({ id: point.id, x: point.analysisRate, y: row.residuals?.[pointIndex] ?? 0 })), color: modelColors[index % modelColors.length], mode: "points" as const }] : []),
